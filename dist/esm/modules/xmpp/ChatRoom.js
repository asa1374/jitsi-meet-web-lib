...
export default class ChatRoom extends Listenable {
    ...
    onPresence(pres) {
        ...
        if (from === this.myroomjid) {
            ...
            if (!this.joined) {
                ...
                if (member.region && ((_a = this.options) === null || _a === void 0 ? void 0 : _a.deploymentInfo)) {
    ...
    processNode(node, from) {
        ...
        try {
            const tagHandlers = (_a = this.presHandlers[node.tagName]) !== null && _a !== void 0 ? _a : [];
    ...
    onMessage(msg, from) {
        ...
        if (txt) {
            ...
            if (type === 'chat') {
                ...
                this.eventEmitter.emit(XMPPEvents.MESSAGE_RECEIVED, from, txt, this.myroomjid, stamp, nick, Boolean(nick), messageId);
            }
        }
    }
    /**
    ...
    onPresenceError(pres, from) {
        ...
        if ($(pres)
            ...
            if (toDomain === this.xmpp.options.hosts.anonymousdomain) {
                ...
                else if ($(pres).find('>error[type="cancel"]>no-main-participants[xmlns="jitsi:visitors"]').length > 0) {
                    type = AUTH_ERROR_TYPES.NO_MAIN_PARTICIPANTS;
                }
                else if ($(pres).find('>error[type="cancel"]>promotion-not-allowed[xmlns="jitsi:visitors"]').length > 0) {
                    type = AUTH_ERROR_TYPES.PROMOTION_NOT_ALLOWED;
                }
                else if ($(pres).find('>error[type="cancel"]>no-visitors-lobby[xmlns="jitsi:visitors"]').length > 0) {
